...
func (OTelSuite) TestAutoSuiteName(ctx context.Context, t *testctx.T) {
	...
	tt := testctx.New(t.Unwrap(), oteltestctx.WithTracing(oteltestctx.TraceConfig[*testing.T]{
	...
	assert.Contains(t, spans[0].Attributes(),
		attribute.String("test.suite.name", "github.com/dagger/otel-go/oteltestctx"))
}
...
func BenchmarkWithTracing(b *testing.B) {
	...
	bb := testctx.New(b, oteltestctx.WithTracing[*testing.B](oteltestctx.TraceConfig[*testing.B]{
		TracerProvider: tracerProvider,
	}))
...
func (OTelSuite) TestInterrupted(ctx context.Context, t *testctx.T) {
	...
	tt := testctx.New(t.Unwrap(),
		...
		oteltestctx.WithTracing(oteltestctx.TraceConfig[*testing.T]{
			TracerProvider: tracerProvider,
		}),
	)
